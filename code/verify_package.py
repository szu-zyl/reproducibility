...
def verify_instances(audit: Audit) -> None:
    ...
    for row in manifest:
        if row["section"] == "5.4":
            ...
            audit.require("Instances", f"{row['instance_id']} service scenarios",
                          np.allclose(source["s"], regenerated["s"], rtol=1e-12, atol=1e-12))
            audit.require("Instances", f"{row['instance_id']} initial lateness scenarios",
                          np.allclose(source["tau"], regenerated["tau"], rtol=1e-12, atol=1e-12))
            audit.require("Instances", f"{row['instance_id']} carried lateness scenarios",
                          np.allclose(source["psi"], regenerated["psi"], rtol=1e-12, atol=1e-12))
